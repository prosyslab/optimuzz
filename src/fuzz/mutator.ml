...
let subst_cast_opt llctx instr =
...
let create_rand_instr llctx llm =
  ...
  | CMP when is_not_ptr ->
      ...
      Some llm
  | OTHER Select ->
      let cond = randget_operand loc (i1_type llctx) in
...
let subst_rand_instr llctx llm =
  ...
  | BINARY ->
      ...
      Some llm
  | CAST ->
      subst_cast_opt llctx instr |> ignore;
      Some llm
  | CMP ->
      let old_cond = icmp_predicate instr |> Option.get in
      subst_cmp llctx instr (OpCls.random_icmp_except old_cond) |> ignore;
      Some llm
  | _ -> None
...
let subst_rand_opd _llctx llm =
  ...
  | (BINARY | MEM _ | CAST | CMP | OTHER Select) when num_operands > 0 ->
