...
module type SEED_POOL = sig
  ...
  type seed = ALlvm.llmodule * conf

  module Cov : CD.COVERAGE

  (* pool functions *)
  val make : ALlvm.llcontext -> unit ALlvm.LLModuleSet.t -> t
  val push : seed -> t -> t
  val push_seq : seed Seq.t -> t -> t
  val pop : t -> seed * t
  val cardinal : t -> int
...
module NaiveSeedPool (Cov : CD.COVERAGE) : SEED_POOL = struct
  module Cov = Cov
  module Optimizer = Oracle.Optimizer (Cov)

  type conf = { covers : bool; score : Cov.DistSet.result }
  type seed = ALlvm.llmodule * conf
  type t = seed Queue.t

  let make_conf covers score = { covers; score }

  let pp_seed fmt (_, conf) =
    Format.fprintf fmt "covers: %b, score: %a" conf.covers Cov.DistSet.pp_result
      conf.score
  ...
  let push_seq seeds pool =
    Queue.add_seq pool seeds;
    pool
  ...
  let make llctx llset =
    ...
    let can_optimize file =
      ...
      match Optimizer.run ~passes:[ "instcombine" ] path with
      | CRASH | INVALID ->
      ...
      |> List.fold_left
           (fun (pool_first, pool_later) file ->
             ...
             if can_optimize file |> not then (pool_first, pool_later)
               ...
               | Some llm -> (
                   match
                     Optimizer.run_for_llm ~passes:[ "instcombine" ] llset llm
                   with
                   | CRASH | INVALID -> (pool_first, pool_later)
                   | VALID cov ->
                       let covers = Cov.cover_target target_path cov in
                       let score =
                         Cov.score target_path cov
                         |> Option.fold ~some:Fun.id
                              ~none:Cov.DistSet.max_metric
                       in

                       let seed = (llm, { covers; score }) in
